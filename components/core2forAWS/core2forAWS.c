...
#if CONFIG_SOFTWARE_FT6336U_SUPPORT
void ft6336u_read(lv_indev_drv_t * drv, lv_indev_data_t * data);
#endif
...
void Core2ForAWS_Display_Init(void) {
    ...
    uint32_t size_in_px = 32 * 320;
    printf("color: %d\n", sizeof(lv_color_t));

    lv_color_t *buf1 = heap_caps_malloc(size_in_px * sizeof(lv_color_t),     MALLOC_CAP_DMA); //Assuming max size of lv_color_t = 16bit, DISP_BUF_SIZE calculated from max horizontal display size 480
    lv_color_t *buf2 = heap_caps_malloc(size_in_px * sizeof(lv_color_t),    MALLOC_CAP_DMA); //Assuming max size of lv_color_t = 16bit, DISP_BUF_SIZE calculated from max horizontal display size 480
    ets_printf("DISP_BUF_SIZE: %d\n", size_in_px);
